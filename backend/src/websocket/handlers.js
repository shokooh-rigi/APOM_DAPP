// WebSocket handlers for APOM DApp - Real-time updates and notifications
...
const setupWebSocketHandlers = (ws) => {
  ...
  ws.send(JSON.stringify({
    type: 'connection',
    message: 'Connected to APOM DApp WebSocket server',
...
const sendMarketDataUpdate = (ws) => {
  const mockMarketData = {
    ...
    defi: {
      price: 1.20 + (Math.random() - 0.5) * 0.08,
  ...
  const mockNotifications = [
    {
      ...
      message: 'You earned 150 APOM tokens from Crypto Warriors!',
      timestamp: new Date().toISOString(),
      read: false
    },
    {
      ...
      message: 'APOM-ETH pool now offers 48.5% APY!',
      timestamp: new Date().toISOString(),
      read: false
    }
  ];
