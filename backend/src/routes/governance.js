...
router.post('/proposals', (req, res) => {
  try {
    ...
    if (!title || !description || !type || !quorum || !creator || !walletAddress) {
    ...
    const newProposal = {
      ...
      discussion: discussion || `https://forum.apom.com/proposal-${Date.now()}`,
...
router.post('/proposals/:id/vote', (req, res) => {
  try {
    const { userId, username, vote, votingPower, walletAddress, reason } = req.body;
...
router.get('/stats', (req, res) => {
  try {
    ...
    const stats = {
      ...
      totalVotingPower: 50000000, // Mock total APOM tokens
      activeVoters: 1250,
...
router.get('/user/:userId/voting-history', (req, res) => {
  try {
    ...
    res.json({
      ...
      totalVotingPower: votingHistory.reduce((sum, v) => sum + v.votingPower, 0),
...
router.get('/active', (req, res) => {
  try {
    const proposals = getGovernanceProposals().filter(proposal => proposal.status === 'Active');
...
router.get('/pending', (req, res) => {
  try {
    const proposals = getGovernanceProposals().filter(proposal => proposal.status === 'Pending');
      ...
      comments: [
        {
          ...
          content: 'This proposal makes a lot of sense for the gaming ecosystem. I support it!',
          timestamp: '2024-04-10T10:30:00Z',
          likes: 12,
          replies: 3,
          authorAvatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=CryptoWarrior'
        },
        {
          id: 'comment_002',
          author: 'DeFiMaster',
          content: 'I have some concerns about the economic impact. Can we discuss the numbers?',
          timestamp: '2024-04-10T11:15:00Z',
          likes: 8,
          replies: 7,
          authorAvatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=DeFiMaster'
        }
      ]
    };
